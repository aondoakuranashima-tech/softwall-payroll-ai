...
export default function LoginPage() {
  ...
  async function submit(e: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      if(token) localStorage.setItem('softwall_token',token);
      router.push('/');
  ...
  return <main style={{minHeight:'100vh',display:'grid',placeItems:'center',background:'#f7f8fa',fontFamily:'Inter,system-ui,sans-serif',padding:24}}>
    <form onSubmit={submit} style={{width:'100%',maxWidth:440,background:'#fff',padding:32,borderRadius:16,border:'1px solid #e5e7eb'}}>
      ...
      <button disabled={loading} type="submit" style={{width:'100%',marginTop:22,padding:14,border:0,borderRadius:9,background:'#111827',color:'#fff',fontWeight:700}}>{loading?'Signing in…':'Log in'}</button>
      <p style={{textAlign:'center'}}>Don't have an account? <Link href="/signup">Sign up</Link></p>
      <p style={{textAlign:'center'}}><Link href="/">Back to home</Link></p>
    </form>
  </main>;
}
